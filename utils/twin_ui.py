...
def run_simulation_engine(target_schools, interventions, time_horizon, target_type):
    ...
    # Initialize models
    markov = MarkovModel()
    monte_carlo = MonteCarloSimulation(n_simulations=500)
    ...
    risk_summary = risk_analyzer.get_division_risk_summary(
        division_name=target_schools[0].get("division", "Division") if target_schools else "Unknown",
        schools_data=target_schools
    )
